...
else if ( isset($_POST['signup']) && isset($_POST['password']) ) {

    $stmts = $conn->prepare("UPDATE users SET password=? where email=?" );
    $psw=hash('sha256', trim($_POST['password']));
    $em=trim($userRow['email']);
    $stmts->bind_param("ss",$psw , $em);
    $res = $stmts->execute();
    $stmts->close();

    $_SESSION['sqlUpdate'] = "update password eseguito";
}
...
if(isset($_SESSION['sqlUpdate'])) {
    echo "<div class='alert alert-success alert-dismissible'><h4><i class='icon fa fa-check'></i> OK!</h4>Password Aggiornata Correttamente</div>";
    unset($_SESSION['sqlUpdate']);
}
...
<!--autocomplete="off"-->
        ...
        <div class="col-md-12">
            ...
            <div class="form-group">
                <h2 class="">Cambio Password: <?php echo $userRow['denominazione'] ?></h2>
            </div>
            ...
            <div class="form-group">
                <button type="submit" class="btn    btn-block btn-primary" name="signup" id="reg">Aggiorna Password</button>
            </div>
